...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("The transform of the camera following the player around.")]
    private Transform m_CameraTransform;

    [SerializeField]
    [Tooltip("A list of all attack and information about them.")]
    ...
    // In order to do anything, we cannot be frozen (timer must be zero)
    private float p_FrozenTimer;

    // Default Color
    private Color p_DefaultColor;

    // Curr amount of health player has
    ...
    private void FixedUpdate()
    {
        ...
        if (p_Velocity.sqrMagnitude > 0) {
            ...
            //supposed to be z not y? 
            Vector2 newRot = new Vector3(Mathf.Cos(angleToRotCam) * camForward.x + Mathf.Sin(angleToRotCam) * camForward.z, 0,
    ...
    private IEnumerator UseAttack(PlayerAttackInfo attack) {
        ...
        StopCoroutine(ChangeColor(p_DefaultColor, 50));
